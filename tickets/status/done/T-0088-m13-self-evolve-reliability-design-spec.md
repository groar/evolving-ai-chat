...
- 2026-03-07: Post-acceptance spec refinement per user feedback: §4 (refinement phase) updated — conversation is strictly functional (no code access, no implementation proposals), context limited to explicit doc allowlist (`refinement-context-docs.json`, STATUS.md first), output is a functional description (goal/current/desired/constraints), refinement replaces direct "Fix with AI" flow entirely (no skip option).
